...
export default function useBlocksConnection() {
  ...
  const checkForWrongExtensionMethodConnection = (connections: [] | string[][]) => {
      ...
      connections.forEach((c) => {
        if (c[0] === matrix._id.toString()) {
          ...
          if (weightBlock && matrix.data.extension === 'fuzzy') {
            if (!fuzzyWeights.map((m) => m.name.toLowerCase()).includes(weightBlock.method.toLowerCase())) {
              showSnackbar(
    ...
    getBlocksOfType(blocks, 'weights').forEach((weights) => {
      ...
      let criteriaSizes = blocks.filter((b) => matrixID.includes(b._id as never)).map((b) => getCriteriaSize(b.data));
      ...
      weightsID.forEach((w) => {
        connections.forEach((c) => {
          if (c[0] === w && c[1] === data?._id.toString()) {
            indexes = indexes.includes(m._id as never) ? indexes : [...indexes, m._id];
          }
        });
      });
    });
